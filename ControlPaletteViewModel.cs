...
{
    public class ControlPaletteViewModel
    {
        public static ControlPaletteViewModel Parse(JsonObject data, IControlPaletteModel paletteModel, IControlPaletteExportProvider exportProvider)
        {
            return new ControlPaletteViewModel(paletteModel, data["Id"].GetOptionalString(), data["Title"].GetOptionalString(), data["Glyph"].GetOptionalString(), exportProvider);
        }
        ...
        private async Task SaveData()
        {
            ...
            else
            {
                ...
                var message = string.Format("ControlPaletteSaveError", file.Path);
                ContentDialog saveFailedDialog = new ContentDialog()
                {
                    CloseButtonText = "ControlPaletteErrorOkButtonCaption",
                    Title = "ControlPaletteSaveErrorTitle",
                    Content = message
                };
        ...
        public async Task LoadData()
        {
            ...
            if (loadedPreset == null)
            {
                ...
                var message = string.Format("ControlPaletteLoadError", file.Path);
                ContentDialog loadFailedDialog = new ContentDialog()
                {
                    CloseButtonText = "ControlPaletteErrorOkButtonCaption",
                    Title = "ControlPaletteLoadErrorTitle",
                    Content = message
                };
        ...
        {
            _exportProvider.SaveExportFile(_exportProvider.GenerateExportData(_paletteModel, this));
        }

        public async Task<string> ExportAsync()
        {
            return await _exportProvider.SaveExportFile(_exportProvider.GenerateExportData(_paletteModel, this));
        }
